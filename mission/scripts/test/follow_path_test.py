...
class FollowPath():
	...
	def __init__(self):
		...
		self.path_seq = ast.literal_eval(json.dumps(self.path_dict.keys()))
	...
	def movebase_client(self):
		...
		if self.path_dict[self.path_seq["task"]] == "pause":
			rospy.loginfo("Pausing for "+str(self.path_dict[self.goal_cnt]["Pause Time"])+" seconds")
			rospy.sleep
	...
	def load_path_database(self):
		with open('/home/jetson/nprobots_ws/src/navigation_yeon/yeon/database/path.json', 'r') as f:
			pdict = json.load(f, object_pairs_hook=OrderedDict)
		return pdict

	def load_task_database(self):
		with open('/home/jetson/nprobots_ws/src/navigation_yeon/yeon/database/task.json', 'r') as f:
